...
export default function TPODashboardPage() {
  const { user } = useAuth();
  const { drives, applications, tpoStats, deleteDrive } = usePlacement();
  ...
  const totalApplicantsAcrossDrives = applications.length;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-5 py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm shadow-lg shadow-brand-500/25 transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {tpoStats.recruitmentFunnel.map((item) => (
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setIsPostModalOpen(true)}
              className="px-3 py-1.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
            >
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
              {filteredDrives.map((drive) => {
                ...
                const driveApplicantsCount = applications.filter(
                  (app) => app.driveId === driveId || app.drive?._id === driveId || app.drive === driveId
                ).length;

                return (
                  <tr key={driveId} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-slate-500 text-[11px]">{drive.role || drive.title}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4 text-center">
                      <Link
                        to={`/tpo/drives/${driveId}/applicants`}
                        className="inline-flex items-center px-3 py-1 rounded-full bg-brand-50 text-brand-700 hover:bg-brand-100 font-bold transition-colors border border-brand-200/60"
                        title="Click to view applicants page"
                      >
                        {driveApplicantsCount}
                      </Link>
                    </td>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          Applicants ({driveApplicantsCount})
                        </Link>
                        <Link
                          to={`/drives/${driveId}`}
                          className="px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:text-brand-600 hover:border-brand-300 transition-colors flex items-center gap-1 text-[11px]"
